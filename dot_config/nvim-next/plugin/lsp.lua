...
vim.api.nvim_create_autocmd("LspAttach", {
	...
	callback = function(args)
		...
		vim.keymap.set("n", "]e", vim.diagnostic.goto_next, { buffer = buffer, desc = _G.desc({ "lsp", "next diagnostic" }) })
		vim.keymap.set("n", "[e", vim.diagnostic.goto_prev, { buffer = buffer, desc = _G.desc({ "lsp", "previous diagnostic" }) })
		vim.keymap.set(
			"n",
			"<leader>li",
		...
		vim.keymap.set("n", "<leader>ls", vim.lsp.buf.signature_help, { buffer = buffer, desc = _G.desc({ "lsp", "signature" }) })
		...
		vim.keymap.set("n", "<leader>lc", vim.lsp.buf.code_action, { buffer = buffer, desc = _G.desc({ "lsp", "code action" }) })
		...
		vim.keymap.set("n", "<leader>ld", vim.lsp.buf.definition, { buffer = buffer, desc = _G.desc({ "lsp", "go to definition" }) })
